...
int main() {
    ...
    while (1){
        if(received_end == 1){
            if(mq_getattr(server_queue, &currentState) == -1) failure_exit("Couldn't read public queue parameters!");
            if(currentState.mq_curmsgs == 0) break;
        }
        if(mq_receive(server_queue, (char*)&message, MSG_SIZE, 0) == -1) {
            failure_exit("Receiving message failed");
        }
        switch (message.type){
            case REGISTER:
    ...
    if(mq_close(server_queue) == -1) failure_exit("Closing server queue failed");
    mq_unlink(server_path);
...
void calc_service(msg message){
    ...
    pclose(calc);
    if(mq_send(client_queue, (char*)&message, MSG_SIZE, 1) == -1) failure_exit("CALC response failed!");
}
...
void time_service(msg message){
    ...
    if(mq_send(client_queue, (char*)&message, MSG_SIZE, 1) == -1) failure_exit("TIME response failed!");
}
...
void stop_service(msg message){
    ...
    else failure_exit("STOP response failed!");
}
...
int get_client_qid(pid_t client_pid){
    ...
    failure_exit("Couldn't find client with this client_pid");
    return -1;
}
